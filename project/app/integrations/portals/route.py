...
from . import schemas
from .integration import PortalsIntegration


portals_router = APIRouter(tags=["Portals V4"], prefix="/portals-v4")


async def _get_parser(
    db_session: AsyncSession = Depends(get_db),
) -> tuple[Account, PortalsIntegration, requests.AsyncSession]:
    """
    Depends for get actual @portals parser
    """
    http_data = await PortalsIntegration.get_parser(PortalsIntegration.auth_expire, PortalsIntegration.market_name)
    ...
    if http_data is None:
        ...
        if len(parsers) == 0:
            logging.getLogger("PortalsParser").error("Не найдено подходящего парсера.")
            raise HTTPException(
...
async def _get_account(
    ...
    """
    Depends for get account's Account, PortalsIntegration and http client by id
    """
    ...
    portals_url = await account.get_webapp_url("portals", telegram_client=telegram_client)
    account_integration = PortalsIntegration(account_model)
    init_data = account_integration.get_init_data_from_url(portals_url)
    http_client = await account_integration.get_http_client(init_data)

    return (account, account_integration, http_client)


@portals_router.post("/", response_model=schemas.MarketSalings)
@cache(expire=15, key_builder=request_without_token_builder)
async def get_salings(
    saling_filter: schemas.PortalsSalingFilter,
    user: models.User = Depends(get_current_user),
    parser_data: tuple[Account, PortalsIntegration, requests.AsyncSession] = Depends(_get_parser),
):
    """
...
@portals_router.get("/balance", response_model=schemas.MarketBalanceResponse)
@cache(expire=60, key_builder=request_key_builder)
async def get_balance(
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, PortalsIntegration, requests.AsyncSession] = Depends(_get_account),
):
    account_integration, http_client = account_data[1], account_data[2]
    return await account_integration.get_balance(http_client)


@portals_router.post("/withdraw", response_model=schemas.MarketActionResponse)
async def withdraw_balance(
    ...
    """Вывести средства с @portals на внутренний баланс пользователя."""
    account_integration, http_client = account_data[1], account_data[2]
    ...
    if result.success:
        ...
        await backend.clear(key=f":get:/portals-v4/deals:[('token', '{user_token}'), ('account_id', '{acc_id}')]")
        await backend.clear(key=f":get:/portals-v4/balance:[('token', '{user_token}'), ('account_id', '{acc_id}')]")

    return result


@portals_router.post("/nft/buy", response_model=schemas.MarketActionResponse)
async def buy(
...
@portals_router.post("/nft/sell", response_model=schemas.MarketActionResponse)
async def sell(
...
@portals_router.post("/nft/cancel", response_model=schemas.MarketActionResponse)
async def cancel(
...
@portals_router.post("/nft/back", response_model=schemas.MarketActionResponse)
async def back(
...
@portals_router.post("/nft/add", response_model=schemas.MarketActionResponse)
async def add_nft(
    ...
    sended = await account.send_gift(telegram_client, reciver_username="GiftsToPortals", msg_id=nft.msg_id)

    return schemas.MarketActionResponse(success=sended)


@portals_router.get("/nfts", response_model=schemas.MarketNFTs)
@cache(expire=60, key_builder=request_key_builder)
async def get_nfts(
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, PortalsIntegration, requests.AsyncSession] = Depends(_get_account),
...
@portals_router.get("/deals", response_model=schemas.MarketDeals)
@cache(expire=60, key_builder=request_key_builder)
async def get_deals(
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, PortalsIntegration, requests.AsyncSession] = Depends(_get_account),
...
@portals_router.get("/offers", response_model=schemas.MarketOffersResponse)
@cache(expire=60, key_builder=request_key_builder)
...
@portals_router.post("/offer/accept", response_model=schemas.MarketActionResponse)
async def accept_offer(
    ...
    if result.success:
        backend = FastAPICache.get_backend()
        await backend.clear(
            key=f":get:/portals-v4/nfts:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )
        await backend.clear(
            key=f":get:/portals-v4/deals:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )
        await backend.clear(
            key=f":get:/portals-v4/offers:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )
        await backend.clear(
            key=f":get:/portals-v4/balance:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )

    return result


@portals_router.post("/offer/cancel", response_model=schemas.MarketActionResponse)
...
@portals_router.post("/offer/new", response_model=schemas.MarketActionResponse)
async def new_offer(
...
@portals_router.post("/floors", response_model=schemas.MarketChartResponse)
...
@portals_router.post("/floor", response_model=schemas.MarketFloorResponse | None)
async def get_floor(
    floor_filter: schemas.MarketFloorFilter,
    db_session: AsyncSession = Depends(get_db),
    user: models.User = Depends(get_current_user),
