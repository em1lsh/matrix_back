...
from . import schemas
from .integration import MrktIntegration


mrkt_router = APIRouter(tags=["Mrkt V4"], prefix="/mrkt-v4")


async def _get_parser(
    db_session: AsyncSession = Depends(get_db),
) -> tuple[Account, MrktIntegration, requests.AsyncSession]:
    """
    Depends for get actual @mrkt parser
    """
    http_data = await MrktIntegration.get_parser(MrktIntegration.auth_expire, MrktIntegration.market_name)
    ...
    if http_data is None:
        ...
        if len(parsers) == 0:
            logging.getLogger("MrktParser").error("Не найдено подходящего парсера.")
            raise HTTPException(
...
async def _get_account(
    ...
    """
    Depends for get account's Account, MrktIntegration and http client by id
    """
    ...
    mrkt_url = await account.get_webapp_url("mrkt", telegram_client=telegram_client)
    account_integration = MrktIntegration(account_model)
    init_data = account_integration.get_init_data_from_url(mrkt_url)
    http_client = await account_integration.get_http_client(init_data)

    return (account, account_integration, http_client)


@mrkt_router.post("/", response_model=schemas.MrktSalingsResponse)
@cache(expire=15, key_builder=request_without_token_builder)
async def get_salings(
    saling_filter: schemas.MrktSalingFilter,
    user: models.User = Depends(get_current_user),
    parser_data: tuple[Account, MrktIntegration, requests.AsyncSession] = Depends(_get_parser),
):
    """
...
@mrkt_router.get("/balance", response_model=schemas.MarketBalanceResponse)
@cache(expire=60, key_builder=request_key_builder)
async def get_balance(
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, MrktIntegration, requests.AsyncSession] = Depends(_get_account),
):
    account_integration, http_client = account_data[1], account_data[2]
    return await account_integration.get_balance(http_client)


@mrkt_router.post("/withdraw", response_model=schemas.MarketActionResponse)
async def withdraw_balance(
    ...
    """Вывести средства с @mrkt на внутренний баланс пользователя."""
    account_integration, http_client = account_data[1], account_data[2]

    logging.getLogger("MrktWithdraw").info(
        "[ROUTE] /mrkt-v4/withdraw called: user_id=%s, account_id=%s, amount=%s nanotons (%.3f TON), memo=%s",
        user.id,
        account_integration.model.id,
        withdraw_data.amount,
        withdraw_data.amount / 1e9,
        withdraw_data.memo,
    )

    # Проверяем баланс на mrkt
    account_balance = await account_integration.get_balance(http_client)
    logging.getLogger("MrktWithdraw").info(
        "[ROUTE] Current mrkt balance: %s nanotons (%.3f TON)",
        account_balance.balance,
        account_balance.balance / 1e9,
    )

    if withdraw_data.amount > account_balance.balance:
        logging.getLogger("MrktWithdraw").warning(
            "[ROUTE] Insufficient mrkt balance: have %s, need %s",
            account_balance.balance,
            withdraw_data.amount,
        )
    ...
    # Выполняем вывод
    logging.getLogger("MrktWithdraw").info("[ROUTE] Calling withdraw_balance integration method...")
    result = await account_integration.withdraw_balance(withdraw_data, http_client)
    logging.getLogger("MrktWithdraw").info(
        "[ROUTE] Integration result: success=%s, detail=%s",
        result.success,
        result.detail,
    )

    if result.success:
        old_balance = user.market_balance
        user.market_balance += withdraw_data.amount
        user_token = user.token
        acc_id = account_integration.model.id

        logging.getLogger("MrktWithdraw").info(
            "[ROUTE] Updating user balance: old=%s, added=%s, new=%s",
            old_balance,
            withdraw_data.amount,
            user.market_balance,
        )
        ...
        logging.getLogger("MrktWithdraw").info("[ROUTE] Database commit successful")

        backend = FastAPICache.get_backend()
        await backend.clear(key=f":get:/mrkt-v4/deals:[('token', '{user_token}'), ('account_id', '{acc_id}')]")
        await backend.clear(key=f":get:/mrkt-v4/balance:[('token', '{user_token}'), ('account_id', '{acc_id}')]")
        logging.getLogger("MrktWithdraw").info("[ROUTE] Cache cleared")
    else:
        logging.getLogger("MrktWithdraw").error(
            "[ROUTE] Withdrawal failed: %s",
            result.detail,
        )
...
@mrkt_router.post("/nft/buy", response_model=schemas.MarketActionResponse)
async def buy(
    ...
    if result.success:
        ...
        backend = FastAPICache.get_backend()
        await backend.clear(key=f":get:/mrkt-v4/nfts:[('token', '{user_token}'), ('account_id', '{acc_id}')]")
        await backend.clear(key=f":get:/mrkt-v4/deals:[('token', '{user_token}'), ('account_id', '{acc_id}')]")
        await backend.clear(key=f":get:/mrkt-v4/balance:[('token', '{user_token}'), ('account_id', '{acc_id}')]")
...
@mrkt_router.post("/nft/sell", response_model=schemas.MarketActionResponse)
async def sell(
    ...
    if result.success:
        backend = FastAPICache.get_backend()
        await backend.clear(
            key=f":get:/mrkt-v4/nfts:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )
        await backend.clear(
            key=f":get:/mrkt-v4/deals:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )

    return result


@mrkt_router.post("/nft/cancel", response_model=schemas.MarketActionResponse)
async def cancel(
...
@mrkt_router.post("/nft/back", response_model=schemas.MarketActionResponse)
async def back(
    ...
    if result.success:
        ...
        await backend.clear(
            key=f":get:/mrkt-v4/balance:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )

    return result


@mrkt_router.post("/nft/add", response_model=schemas.MarketActionResponse)
async def add_nft(
    nft_id: str,  # Строка для избежания переполнения int
    db_session: AsyncSession = Depends(get_db),
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, MrktIntegration, requests.AsyncSession] = Depends(_get_account),
):
    ...
    try:
        ...
        raise HTTPException(status_code=http.HTTPStatus.BAD_REQUEST, detail="Invalid NFT ID format.")

    nft = await db_session.execute(
        select(models.NFT).where(
            models.NFT.user_id == user.id, models.NFT.account_id == account.model.id, models.NFT.id == nft_id_int
        )
    )
    ...
    sended = await account.send_gift(telegram_client, reciver_username="mrktbank", msg_id=nft.msg_id)

    return schemas.MarketActionResponse(success=sended)


@mrkt_router.get("/nfts", response_model=schemas.MarketNFTs)
@cache(expire=60, key_builder=request_key_builder)
async def get_nfts(
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, MrktIntegration, requests.AsyncSession] = Depends(_get_account),
...
@mrkt_router.get("/deals", response_model=schemas.MarketDeals)
@cache(expire=60, key_builder=request_key_builder)
async def get_deals(
    user: models.User = Depends(get_current_user),
    account_data: tuple[Account, MrktIntegration, requests.AsyncSession] = Depends(_get_account),
...
@mrkt_router.get("/offers", response_model=schemas.MarketOffersResponse)
@cache(expire=60, key_builder=request_key_builder)
...
@mrkt_router.post("/offer/accept", response_model=schemas.MarketActionResponse)
async def accept_offer(
    ...
    if result.success:
        ...
        await backend.clear(
            key=f":get:/mrkt-v4/offers:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )
        await backend.clear(
            key=f":get:/mrkt-v4/balance:[('token', '{user.token}'), ('account_id', '{account_integration.model.id}')]"
        )
...
@mrkt_router.post("/offer/cancel", response_model=schemas.MarketActionResponse)
...
@mrkt_router.post("/offer/new", response_model=schemas.MarketActionResponse)
async def new_offer(
...
@mrkt_router.post("/floors", response_model=schemas.MarketChartResponse)
async def get_floors(
    ...
    """
    Получить историю изменения флора по модели/коллекции
    """
    floors_lte = datetime.datetime.now() + datetime.timedelta(days=int(floor_filter))
    market_id = await crud.get_market_id_by_title("mrkt", db_session)
...
@mrkt_router.post("/floor", response_model=schemas.MarketFloorResponse | None)
async def get_floor(
    floor_filter: schemas.MarketFloorFilter,
    db_session: AsyncSession = Depends(get_db),
    user: models.User = Depends(get_current_user),
